...
// Find specific values for dropdowns
const defaultLocation = countries.find((c) => c.name === 'Abuja, Nigeria')?.countryCode || '';
const defaultCurrency = currencies.find((c) => c.name.includes('Nigerian Naira'))?.key || '';
const defaultLanguage = Languages.find((l) => l.name === 'English')?.value || '';
const defaultFiscalYear = FiscalYear.find((f) => f.name === 'January - December')?.key || '';
const defaultTimezone = 'Africa/Lagos'; // Timezone value
...
export default function SetupOrganizationForm({ isSubmitting }) {
  return (
